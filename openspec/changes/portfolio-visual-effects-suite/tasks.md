...
- [x] 0.7 Modify `components/common/CyberNebula.tsx` and `components/common/WaterSurface3D.tsx` — consume `useMotionCapability`, return `null` under reduced motion; fix `WaterSurface3D`'s narrower `"(hover: none)"` query to the shared constant (R-2). Coverage added post-review: `CyberNebula.test.tsx` and `WaterSurface3D.test.tsx` assert the reduced-motion gate returns `null` (no `.hero-nebula`/`.about-water-3d` wrapper) plus a sanity-control render on the normal desktop path.
- [x] 0.8 Modify `styles/globals.css` — consolidate `.about-water-3d` rule into shared reduced-motion contract.
- [x] 0.9 Create `.lighthouserc.json` + `.github/workflows/lighthouse.yml` — local prod build, `numberOfRuns:3`, blocking assertions (perf/a11y/seo/best-practices ≥0.9, LCP≤2500ms, CLS≤0.1, TBT≤200ms); add `@lhci/cli` to `package.json`. Fixed post-review: added `permissions: contents: read` at workflow level (least-privilege, matches `pages-redirect.yml` pattern).
- [x] 0.10 Verify: `pnpm lint`/`typecheck`/`test` green; Lighthouse workflow present and green on `main` before Phase 1 merges (Req: Baseline Precedes Effects). Coverage added post-review: `usePrefersReducedMotion`'s `getServerSnapshot` is now an exported, directly unit-tested pure function (SSR default can't be exercised via `renderHook()` in jsdom); `tests/matchMedia.ts` (shared mock used by 7+ test files) now has its own dedicated test file covering default value, `setMediaQueryState`, `resetMediaQueryState`, and change-listener notification.
