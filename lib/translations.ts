...
export const translations: Translations = {
  ...
  "job8.desc": {
    es: "Desarrollo frontend con Angular, TypeScript, Angular Material y Bootstrap, participando en la migración incremental del producto desde AngularJS a Angular 2 y en las sucesivas actualizaciones de versión mayor posteriores. Integración de Google Maps API y diseño responsivo con CSS y SCSS. Personalización de plantillas y componentes de Angular Material, sobrescribiendo los estilos por defecto de la librería con SCSS y selectores ::ng-deep de Angular para ajustar las interfaces a los diseños requeridos. Responsable de adaptar el frontend a la migración del backend a microservicios, consumiendo un AWS API Gateway que enrutaba cada petición a su función Lambda correspondiente y unificando la capa HTTP de la aplicación bajo la convención de endpoints del gateway. Trabajo con metodologías ágiles en entorno Linux, dentro de un equipo de 10 desarrolladores organizado por áreas —frontend, backend y mobile— cada una con su líder técnico, desarrollando dentro de un único contenedor Docker compartido que cargaba todo el equipo de frontend para eliminar las diferencias de entorno entre desarrolladores, y colaborando mediante merge requests en GitLab.",
    en: "Frontend development with Angular, TypeScript, Angular Material, and Bootstrap, contributing to the product's incremental migration from AngularJS to Angular 2 and to the successive major-version upgrades that followed. Google Maps API integration and responsive design with CSS and SCSS. Customized third-party templates and Angular Material components by overriding the library's default styles with SCSS and Angular's ::ng-deep selectors to match the required designs. Owned the frontend adaptation to the backend's migration to microservices, consuming an AWS API Gateway that routed each request to its corresponding Lambda function and unifying the application's HTTP layer under the gateway's endpoint convention. Agile methodologies in a Linux environment, within a 10-developer team organized by area — frontend, backend, and mobile — each with its own tech lead, developing inside a single shared Docker container that the whole frontend team loaded to eliminate environment drift between developers, and collaborating through GitLab merge requests.",
  },
